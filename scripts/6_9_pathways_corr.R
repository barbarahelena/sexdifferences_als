## Pathways - microbes in Israel cohort
...
dim(mb)
microb <- read.csv("results/humancohort/wilcoxons_microbes.csv") |> 
...
df <- readRDS("data/human_cohort/human_als_pathways.RDS") %>%
...
ggsave("results/humancohort/heatmap_microbes_pathways_top15.pdf", plot = as_ggplot(heatmap_top_grob),
       width = 12, height = 9)
...
# Save results
write.csv(sex_diff_results, "results/humancohort/pathways_sexdiff_wilcoxon.csv", row.names = FALSE)
...
if(length(res_box) > 0){
    ...
    ggsave("results/humancohort/rhamnose_pathways_sexdifferences_fdr_sig.pdf",
           width = 4*ncol_plot, height = 4*nrow_plot)

    cairo_pdf("results/humancohort/rhamnose_pathways_sexdifferences_fdr_sig_cairo.pdf",
              width = 4*ncol_plot, height = 4*nrow_plot, family = "Helvetica")
    print(paths_sexdiff)
    dev.off()

    svg(filename = "results/humancohort/rhamnose_pathways_sexdifferences_fdr_sig.svg",
        width = 4*ncol_plot, height = 4*nrow_plot)
    print(paths_sexdiff)
    dev.off()
} else {
